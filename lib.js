//Version 5
 ...
 //msg.voice = voices.filter(function(voice) { return voice.name == 'Google UK English Male'; })[0];
	...
	//This triggers everytime the syth module finishes talking.  This way I have a variable
	//that keeps track of when the system is talking so I don't confuse my game
	msg.onend = function(){
		talking = false;
	}
	
	//This runs everytime there is a change to the synth voices, it triggers once the
	// voice synth modules have bee loaded (in other words downloaded from google heaven aka the mystery ether of the universe)
	...
	//Function I made up, just give it a string, and speech comes out
	//Remember, if you spam this function it won't say stuff simultaneously, it will
	//form a queue, thats why I use 'var talking' to keep track so that doesn't happen
	function speak(m){
		if ('speechSynthesis' in window) {		// Check for Synthesis support. 
			msg.text = m	//Feed the message into the speech config Object
			speechSynthesis.speak(msg); //Send the message to the speech Synth module
			talking = true; //Used in my games and stuff because it can only say 1 thing at a time
		}
	}
